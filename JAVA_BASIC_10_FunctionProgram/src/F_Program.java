 ...
 * 1. 함수형 프로그래밍
 * 1.1 명령형(imperative)
 * 1.1.1 프로그램의 상태와 상태를 변경시키는 구문의 관점에서 연산을 설명하는 프로그래밍 패러다임의 일종
 * 1.1.2 포트란, ALGOL, C언어 등이 명령형 프로그래밍 언어의 일종
 * 1.2 선언형 (declaretive)
 * 1.2.1 프로그램이 무엇을 해야 할지를 나타내는 경우를 선언형이라고 한다.
 *    -> 컴퓨터 화면에 웹 페이지를 나타낼 때 "어떤 방법"으로 페이지를 나타내야 하는지 보다 
 *       제목, 본문, 그림 등과 같이 "무엇"을 화면에 나타내야 할지를 고민하는 것이 선언형 프로그래밍이다.
 * 1.2.2 프로그램이 함수형, 논리형, 제한형 프로그래밍 언어 등으로 작성된 경우에 선언형이라고 한다.
 * 	  -> select * from users where country='서울'; 에서 정보를 어떻게(How) 가져오는 것보다 어떤 정보(What)를 가져올지에 대해 더 관심이 많다.
 * 
 * 1.3 명령형 프로그래밍은 무엇을 어떻게(How) 할 것인가에 가깝고, 선언형 프로그래밍은 무엇을(What) 할 것인가에 가깝다.
 * 1.3.1 예시
 *    -> 첫번째 예시
